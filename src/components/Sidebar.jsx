...
import { Home, Calendar, Clock, BookOpen, Trophy, Calculator, FileText, LogOut, Database, Shield, Link as LinkIcon, Library, Building2, ChevronDown, ChevronUp, TrendingUp, Menu, X, GraduationCap } from 'lucide-react';
...
const Sidebar = () => {
  ...
  if (userProfile?.role === 'collegeadmin') {
    menuItems.push({ icon: Building2, label: 'College Admin', path: '/collegeadmin' });
  }
  ...
  return (
    <>
          ...
          <div className="px-6 space-y-1 max-h-64 overflow-y-auto">
            {enrolledCourses.length > 0 ? (
              enrolledCourses.map((course, index) => (
                ...
                >
                  <div className={`w-2 h-2 rounded-full ${getCourseColor(index)}`}></div>
                  <span className="truncate">{course.name}</span>
                </Link>
              ))
            ) : (
              <p className="text-sm text-gray-500 py-2">No courses enrolled</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="border-t border-gray-800 p-4">
        <Link to="/profile" className="flex items-center gap-3 mb-3 hover:bg-gray-800 p-2 rounded-lg transition-colors">
          <Avatar
