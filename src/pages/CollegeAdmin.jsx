...
import { collection, getDocs, query, where, doc, updateDoc, deleteDoc, getDoc, addDoc } from 'firebase/firestore';
import { db } from '../firebase/config';
import { useAuth } from '../context/AuthContext';
import { Building2, Users, BookOpen, Link as LinkIcon, Plus, X, Save, Trash2, Shield, UserPlus, UserMinus, Search } from 'lucide-react';
...
const CollegeAdmin = () => {
  ...
  useEffect(() => {
    if (userProfile?.role === 'superadmin' || (userProfile?.role === 'collegeadmin' && userProfile?.collegeId)) {
      fetchCollegeData();
    } else {
      setLoading(false);
    }
  ...
  const deleteCourse = async (courseId) => {
    if (!window.confirm('Are you sure you want to delete this course? All folders and enrollments will be removed.')) {
      return;
    }
    ...
    try {
      ...
      // Update college's courses array
      const collegeRef = doc(db, 'colleges', college.id);
      const updatedCourses = (college.courses || []).filter(id => id !== courseId);
      await updateDoc(collegeRef, { courses: updatedCourses });
      ...
      usersSnapshot.docs.forEach(userDoc => {
        ...
        const adminCourses = (userData.adminCourses || []).filter(id => id !== courseId);
        
        if (userData.enrolledCourses?.includes(courseId) || userData.adminCourses?.includes(courseId)) {
          updatePromises.push(
  ...
  const filteredStudents = collegeStudents.filter(student =>
    student.displayName?.toLowerCase().includes(searchStudents.toLowerCase()) ||
  ...
  if (userProfile?.role !== 'superadmin' && userProfile?.role !== 'collegeadmin') {
    return (
    ...
    <div className="min-h-screen bg-gray-50 pb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-800">{collegeStudents.length}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-800">{college?.links?.length || 0}</p>
              </div>
                ...
                <div key={course.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  {admins.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-sm font-medium text-gray-700 mb-2">Course Admins:</p>
                      <div className="flex flex-wrap gap-2">
                        {admins.map(admin => (
                          <div key={admin.id} className="flex items-center gap-2 px-3 py-1 bg-blue-50 rounded-full text-sm">
                            <Avatar photoURL={admin.photoURL} displayName={admin.displayName} size="xs" />
                            <span className="text-gray-700">{admin.displayName}</span>
                            <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
              <h3 className="text-xl font-bold text-gray-800">Edit Quick Links</h3>
              <button onClick={() => setShowLinksModal(false)} className="text-gray-500 hover:text-gray-700">
                <X size={24} />
              </button>
            </div>
      ...
      {showAdminModal && selectedCourse && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
              <h3 className="text-xl font-bold text-gray-800">Manage Admins - {selectedCourse.name}</h3>
            ...
            <div className="p-6">
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {filteredStudents.map((student) => {
                  const isAdmin = student.adminCourses?.includes(selectedCourse.id);
                  return (
                    <div key={student.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                      <div className="flex items-center gap-3">
                        <Avatar photoURL={student.photoURL} displayName={student.displayName} size="md" />
                        <div>
                          <div className="font-medium text-gray-800">{student.displayName}</div>
                          <div className="text-sm text-gray-500">{student.email}</div>
                        </div>
                      </div>
                      {isAdmin ? (
                        <button
                          onClick={() => removeAdmin(student.id, selectedCourse.id)}
                        ...
                        <button
                          onClick={() => makeAdmin(student.id, selectedCourse.id)}
