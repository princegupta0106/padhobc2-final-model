...
import { Trophy, TrendingUp, Award, Medal, Crown, Zap, ArrowLeft, HelpCircle } from 'lucide-react';
...
const Leaderboard = () => {
  ...
  const fetchLeaderboards = async () => {
    try {
      ...
      const usersData = usersSnapshot.docs.map(doc => {
        ...
        const totalMinutes = (data.dailyTime || []).reduce((sum, day) => sum + (day.totalMinutes || 0), 0);
      ...
      const userXPRank = sortedByXP.findIndex(u => u.id === userProfile.uid) + 1;
  ...
  const getRankMedal = (rank) => {
    ...
    if (rank === 3) return { icon: Medal, color: 'text-orange-600', bg: 'bg-orange-50' };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-lg shadow-sm text-center max-w-md">
          ...
          <p className="text-gray-600">You need to be part of a college to view the leaderboard.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800">College Leaderboard</h1>
          </div>
          <p className="text-gray-600 text-sm">Compete with your peers and climb the ranks!</p>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="flex items-center gap-1 justify-end">
                <Zap size={24} />
                <p className="text-3xl font-bold">{topUsers.find(u => u.id === userProfile.uid)?.xp || 0}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-gray-800">Top Users</h2>
            </div>
            <button
          ...
          {showXPInfo && (
            <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-sm text-gray-700">
                <span className="font-semibold">XP Formula:</span> 10 XP per contribution + 2 XP per minute spent learning
              </p>
            </div>
          )}
          ...
          <div className="space-y-2">
            {topUsers.map((user, index) => {
              ...
              return (
                ...
                >
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full flex-shrink-0 ${medal.bg}`}>
                    {rank <= 3 ? (
