...
import { collection, getDocs, query, where, doc, updateDoc, deleteDoc, getDoc, arrayUnion } from 'firebase/firestore';
import { db, storage } from '../firebase/config';
import { ref, deleteObject, uploadBytes, getDownloadURL } from 'firebase/storage';
import { useAuth } from '../context/AuthContext';
import { BookOpen, Users, Trash2, FolderOpen, FileText, Shield, Search, CheckCircle, XCircle, Clock, Star, Bell, Upload, Newspaper, Plus, Paperclip } from 'lucide-react';
...
const Admin = () => {
  ...
  const [newsForm, setNewsForm] = useState({ courseId: '', title: '', content: '' });
  const [newsAttachments, setNewsAttachments] = useState([]);

  useEffect(() => {
    if (!hasFetched && (userProfile?.role === 'superadmin' || (userProfile?.role === 'admin' && userProfile?.adminCourses?.length > 0))) {
      fetchManagedCourses();
      fetchNotifications();
      setHasFetched(true);
    } else if (!userProfile) {
  ...
  const fetchManagedCourses = async () => {
    try {
      ...
      if (userProfile?.role === 'superadmin') {
        ...
        const coursesPromises = (userProfile?.adminCourses || []).map(courseId => 
        ...
        adminCourses.map(async (course) => {
          ...
          folders.forEach(folder => {
            ...
            if (folder.moderationStatus === 'pending') {
              console.log('Found pending folder:', folder.name, folder.moderationStatus);
              pendingFolders.push({
                ...folder,
            ...
            if (folder.files) {
              folder.files.forEach((file, index) => {
                if (file.moderationStatus === 'pending') {
                  console.log('Found pending file:', file.name, file.moderationStatus);
                  pendingFiles.push({
                    ...file,
                    folderId: folder.id,
      ...
      console.log('Total pending uploads:', pendingFolders.length + pendingFiles.length);
  ...
  const moderateFromNotification = async (notification, action) => {
    try {
      ...
      await moderateFolder(folderId, action === 'accept' ? 'accepted' : 'rejected');
  ...
  const deleteFolder = async (courseId, folderId, uploaderId, filesCount) => {
    if (!window.confirm('Are you sure you want to permanently delete this folder? All files will be removed.')) {
      return;
    }
  ...
  const moderateFolder = async (folderId, status) => {
    const loadingToast = toast.loading(`${status === 'accepted' ? 'Accepting' : 'Rejecting'} folder...`);
  ...
  const toggleImportant = async (folderId, currentStatus) => {
    ...
    try {
      ...
      toast.success(`Folder marked as ${!currentStatus ? 'important' : 'normal'}`, { id: loadingToast });
      fetchManagedCourses();
    } catch (error) {
  ...
  const moderateFile = async (folderId, fileIndex, status) => {
    const loadingToast = toast.loading(`${status === 'accepted' ? 'Accepting' : 'Rejecting'} file...`);
    ...
    try {
      ...
      toast.success(`File ${status === 'rejected' ? 'rejected and removed' : status}`, { id: loadingToast });
      fetchManagedCourses();
    } catch (error) {
      console.error('Moderation error:', error);
  ...
  const handlePostNews = async (e) => {
    ...
    if (!newsForm.courseId || !newsForm.title.trim() || !newsForm.content.trim()) {
    ...
    try {
      ...
      if (newsAttachments.length > 0) {
        ...
        for (let i = 0; i < newsAttachments.length; i++) {
          ...
          const attachmentRef = ref(storage, `news/${newsForm.courseId}/${timestamp}_${i}_${file.name}`);
  ...
  if (userProfile?.role !== 'superadmin' && (userProfile?.role !== 'admin' || !userProfile?.adminCourses?.length)) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <Shield size={64} className="text-gray-300 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Access Denied</h2>
          <p className="text-gray-600">You don't have admin access to any courses.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6 pr-16 lg:pr-8">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-3xl font-bold text-gray-800">Admin Panel</h1>
                <p className="text-xs md:text-sm text-gray-600 hidden sm:block">Manage courses you are assigned to</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 md:p-6 mb-6">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {notifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`bg-white rounded-lg p-4 border ${notification.read ? 'border-gray-200 opacity-60' : 'border-blue-300'}`}
                >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-semibold text-gray-800">
                          New {notification.type === 'bulk_upload' ? 'Bulk Upload' : 'Upload'}
                        </span>
                        {!notification.read && (
                      ...
                      <p className="text-gray-700 mb-2">
                        <button
                          onClick={() => navigate(`/user/${notification.uploaderId}`)}
                          className="font-medium text-blue-600 hover:text-blue-800 hover:underline"
                        >
                          {notification.uploaderName}
                        </button>
                        {notification.type === 'bulk_upload' ? (
                          <span> uploaded <span className="font-medium">{notification.folderCount} folders</span> with <span className="font-medium">{notification.fileCount} files</span> to </span>
                        ) : (
                          <span> uploaded {notification.itemType === 'folder' ? 'a folder' : 'files'} ({notification.fileCount} files) to </span>
                        )}
                        <button
                          onClick={() => navigate(`/course/${notification.courseId}`)}
                          className="font-medium text-blue-600 hover:text-blue-800 hover:underline"
                        >
                      ...
                      {notification.itemName && (
                        <p className="text-sm text-gray-600 mb-2">
                          Folder: <span className="font-medium">{notification.itemName}</span>
                        </p>
                      )}
                      ...
                      {!notification.read && (
                        <div className="flex gap-2 mt-3">
                          <button
                            onClick={() => moderateFromNotification(notification, 'accept')}
                          ...
                          <button
                            onClick={() => moderateFromNotification(notification, 'reject')}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        {item.type === 'file' && (
                          <p>Folder: <span className="font-medium">{item.folderName}</span></p>
                        )}
                        {item.type === 'folder' && (
                          <p>Files: <span className="font-medium">{item.files?.length || 0}</span></p>
                        )}
                        <p>
                          Uploaded by:{' '}
                          {item.uploadedById ? (
                            <button
                              onClick={() => navigate(`/user/${item.uploadedById}`)}
                              className="font-medium text-blue-600 hover:text-blue-800 hover:underline"
                            >
                              {item.uploadedBy || 'Unknown'}
                            </button>
                          ) : (
                            <span className="font-medium">{item.uploadedBy || 'Unknown'}</span>
                          )}
                        </p>
                        <p className="text-xs text-gray-500">
                      ...
                      <button
                        onClick={() => {
                          if (item.type === 'folder') {
                            ...
                            moderateFile(item.folderId, item.fileIndex, 'accepted');
                          }
                        }}
                      ...
                      <button
                        onClick={() => {
                          if (item.type === 'folder') {
                            ...
                            moderateFile(item.folderId, item.fileIndex, 'rejected');
                          }
                        }}
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg md:text-xl font-bold text-gray-800">Course News</h2>
            </div>
            <button
          ...
          {showNewsForm && (
            <form onSubmit={handlePostNews} className="bg-gray-50 rounded-lg p-4 space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewsForm({ ...newsForm, courseId: e.target.value })}
                ...
                <div className="space-y-3">
                  <label className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 cursor-pointer transition-colors w-fit">
                    ...
                    <input
                      type="file"
                      multiple
                      onChange={(e) => setNewsAttachments([...newsAttachments, ...Array.from(e.target.files)])}
                      className="hidden"
                    ...
                    <div className="space-y-2">
                      {newsAttachments.map((file, index) => (
                        <div key={index} className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded">
                          ...
                          <button
                            type="button"
                            onClick={() => setNewsAttachments(newsAttachments.filter((_, i) => i !== index))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-6 mb-6 md:mb-8">
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-xs md:text-sm mb-1">Managed Courses</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-800">{coursesData.length}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-xs md:text-sm mb-1">Total Folders</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-800">
                  {coursesData.reduce((sum, course) => sum + course.foldersCount, 0)}
                </p>
              </div>
              <FolderOpen size={32} className="text-green-500 md:w-10 md:h-10" />
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-xs md:text-sm mb-1">Total Files</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-800">
                  {coursesData.reduce((sum, course) => sum + course.filesCount, 0)}
                </p>
              </div>
                ...
                <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-3 md:p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <h2 className="text-lg md:text-xl font-bold mb-1.5 md:mb-2 truncate">{course.name}</h2>
                      ...
                      {course.folders.map((folder) => (
                        ...
                        >
                          <div className="flex items-center gap-4 flex-1">
                            <FolderOpen size={20} className={folder.isImportant ? "text-yellow-500" : "text-blue-500"} />
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-gray-500">
                                {folder.files?.length || 0} files • Uploaded by {folder.uploadedBy}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => toggleImportant(folder.id, folder.isImportant)}
                              className="opacity-0 group-hover:opacity-100 p-2 hover:bg-yellow-100 rounded-lg transition-all"
                              title={folder.isImportant ? "Mark as normal" : "Mark as important"}
                            >
                              <Star size={16} className={folder.isImportant ? "text-yellow-500 fill-yellow-500" : "text-gray-400"} />
                            </button>
                            <button
                              onClick={() => deleteFolder(course.id, folder.id, folder.uploadedById, folder.files?.length || 0)}
