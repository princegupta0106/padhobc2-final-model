...
const CoursePage = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (!courseData.collegeId) {
        ...
        if (!hasAccess) {
          toast.error(
            "Access denied. This course is not available in your college."
          );
          navigate("/");
          return;
        }
  ...
  const canDeleteFromCourse = () => {
    ...
    if (
      ...
      userProfile?.adminCourses?.includes(courseId)
    ) {
      return true;
    }
  ...
  const deleteFolder = async (folderId) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this folder? All files will be removed."
      )
    ) {
      return;
    }
  ...
  const fetchFolderDetails = async (folderId) => {
    ...
    try {
      ...
      if (folderDoc.exists()) {
        ...
        setResources((prev) =>
          prev.map((r) => (r.id === folderId ? { ...r, ...folderData } : r))
        );
      }
    } catch (error) {
  ...
  const handleAddNews = async () => {
    ...
    try {
      ...
      for (const file of newsFiles) {
        const fileRef = ref(
          storage,
          `news/${courseId}/${Date.now()}_${file.name}`
        );
    ...
    return resources.map((resource) => {
      ...
      return (
        <div key={resource.id} className="mb-1">
          <div
            className={`w-full flex items-center gap-2 px-4 py-2 rounded-lg transition-colors group relative ${
              isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"
            } ${isPending ? "border-l-2 border-l-gray-400" : ""} ${
              resource.isImportant
                ? isDarkMode
                  ? "border-l-2 border-l-purple-400 bg-purple-900/20"
                  : "border-l-2 border-l-purple-300 bg-purple-50/30"
                : ""
            }`}
          >
            <button
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate(
                  `/course/${courseId}/upload?folder=${encodeURIComponent(
                    resource.name
                  )}`
                );
              }}
          ...
          {isExpanded && (
            <div
              className={`ml-6 border-l-2 pl-2 mt-1 ${
              ...
              {fileCount === 0 ? (
                ...
                <div className="space-y-1">
                  {resource.files.map((file, index) => {
                    ...
                    const uploaderName =
                      file.uploadedBy || resource.uploadedBy || "Unknown";
                    const uploaderId =
                      file.uploadedById || resource.uploadedById;
                    ...
                    return (
                      <div
                        key={index}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors group relative cursor-pointer ${
                          isDarkMode ? "hover:bg-gray-700" : "hover:bg-gray-100"
                        } ${
                          isFilePending ? "border-l-2 border-l-gray-400" : ""
                        }`}
                        onClick={() => {
                          if (isViewable) {
                            window.open(
                              `/viewer?url=${encodeURIComponent(
                                file.url
                              )}&name=${encodeURIComponent(file.name)}&size=${
                                file.size
                              }&courseId=${courseId}`,
                              "_blank"
                            );
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div
              className={`rounded-lg shadow-sm p-4 md:p-6 min-h-[400px] ${
                ...
                <div className="space-y-1">
                  {course.news.map((news, index) => {
                    ...
                    return (
                      <div key={index}>
                        <div
                          className={`px-4 py-3 rounded-lg transition-colors group border relative ${
                            isDarkMode
                        ...
                        >
                          ...
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-2 text-xs text-gray-500 mt-1 ml-9">
                            <span>
                              by {news.postedBy || news.createdBy || "Admin"}
                            </span>
                            <span>•</span>
                            <span>
                              {new Date(
                                news.postedAt || news.createdAt
                              ).toLocaleDateString()}
                            </span>
                          </div>
                        </div>
